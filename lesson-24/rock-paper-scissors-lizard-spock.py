...
defeated_by = {"rock":["scissors", "lizard"], "scissors":["paper","lizard"],
...
while round_counter != num_of_rounds:
    ...
    user_choice = input("Choose an element(rock, paper, scissors, lizard, spock): ")
    comp_choice = choice(list(defeated_by.keys()))
    print(comp_choice)

    for key,value in defeated_by.items():
        if user_choice == key and comp_choice in value:
            print("You win!\n")
            round_counter += 1
            win_counter += 1
            break
        elif user_choice in value and comp_choice == key:
            print("You lose!\n")
            round_counter += 1
            lose_counter += 1
            break
